		...
		if ( sdWorld.is_server )
		{
			...
			if ( this.detonation_in <= 0 )
			{
				...
				setTimeout(()=>{//Just in case
					let portal = new sdRift({x: this.x  - 5, y:this.y - 5, type:5 });
					sdEntity.entities.push( portal );
					let portal2 = new sdRift({x: this.x + 5, y:this.y + 5, type:5 });
					sdEntity.entities.push( portal2 );
					let portal3 = new sdRift({x: this.x + 5, y:this.y - 5, type:5 });
					sdEntity.entities.push( portal3 );
					let portal4 = new sdRift({x: this.x - 5, y:this.y + 5, type:5 });
