		...
		*/
		...
		sdGun.classes[ sdGun.CLASS_SABER = 16 ] = { // Original weapon idea & pull request by Booraz149 ( https://github.com/Booraz149 ), image editing & sound effects by Eric Gurt
			...
			upgrades: AddGunDefaultUpgrades(
			 ...
			 AddRecolorsFromColorAndCost( [],
			 '#2d2d2d', 30, 'Dark' ),
			 '#0000ff', 30, 'Outer Energy' ),
			 '#ffffff', 30, 'Inner Energy' ),
			 '#484848', 30, 'Body' ),
			 '#555555', 30, 'Hand' ) )
		};
		...
		sdGun.classes[ sdGun.CLASS_SHOVEL_MK2 = 36 ] = { // Sprite made by LazyRain
			...
			upgrades: AddGunDefaultUpgrades(
			 ...
			 AddRecolorsFromColorAndCost( [],
			 '#2d2d2d', 5, 'Dark' ),
			 '#0000ff', 5, 'Outer Energy' ),
			 '#ffffff', 5, 'Inner Energy' ),
			 '#484848', 5, 'Body' ),
			 '#555555', 5, 'Hand' ) )
		};
		...
		sdGun.classes[ sdGun.CLASS_KVT_MISSILE_LAUNCHER = 68 ] = // sprite by Ghost581
		{
			...
			upgrades: AddGunDefaultUpgrades(
			 ...
			 '#373737', 20, 'Bright' ),
			 '#1a1a1a', 20, 'Muzzle' ),
			 '#626262', 20, 'Power Barrel' ),
			 '#484848', 20, 'Power Barrel 2' ) )
		};
		...
		sdGun.classes[ sdGun.CLASS_GRENADE_LAUNCHER_MK2 = 74 ] = 
		{
			...
			upgrades: AddGunDefaultUpgrades(
			 ...
			 '#202020', 15, 'Bright' ),
			 '#404040', 15, 'Hand' ),
			 '#505050', 15, 'Grenade Box' ),
			 '#303030', 15, 'Grenade Box 2' ) )
		};
		...
		sdGun.classes[ sdGun.CLASS_KVT_AVRS = 77 ] = // sprite by Ghost581
		{
			...
			upgrades: AddGunDefaultUpgrades(
			 ...
			 '#232323', 20, 'Scope' ),
			 '#212121', 20, 'Muzzle' ),
			 '#282828', 20, 'Power Barrel' ),
			 '#323232', 20, 'Power Barrel 2' ) )
		};
		...
		sdGun.classes[ sdGun.CLASS_COUNCIL_WORM_GUN = 114 ] = // Sprite by Flora/Gravel
		{
			...
			onShootAttempt: ( gun, shoot_from_scenario )=>
			{
				...
				else
				{
					...
					if ( gun._held_by._key_states.GetKey( 'Mouse1' ) )
					{
						gun._held_by._auto_shoot_in = ( gun._held_by.stim_ef > 0 ) ? ( gun.extra[ ID_FIRE_RATE ] ) : ( 3 * gun.extra[ ID_FIRE_RATE ] ); // Faster rate of fire when shooting more
						gun._held_by.matter -= 6;
					}
				}
				return true;
			...
			onMade: ( gun, params )=> // Should not make new entities, assume gun might be instantly removed once made
			{
				if ( !gun.extra )
				{
					...
					gun.extra[ ID_DAMAGE_MULT ] = 1;
					gun.extra[ ID_FIRE_RATE ] = 1;
					gun.extra[ ID_TEMPERATURE_APPLIED ] = 200;
					gun.extra[ ID_RECOIL_SCALE ] = 1;
					gun.extra[ ID_DAMAGE_VALUE ] = 28; // Damage value of the bullet, needs to be set here so it can be seen in weapon bench stats
				}
			},
		...
		sdGun.classes[ sdGun.CLASS_SHURG_SNIPER = 115 ] = 
		{
			...
			matter_cost: 120,
			min_build_tool_level: 9,
			projectile_properties: { _damage: 115, penetrating:true, _dirt_mult: -0.5 },
			projectile_properties_dynamic: ( gun )=>{ 
		...
		sdGun.classes[ sdGun.CLASS_CRYOGUN = 118 ] = 
		{
			...
			onMade: ( gun, params )=> // Should not make new entities, assume gun might be instantly removed once made
			{
				if ( !gun.extra )
				{
					...
					gun.extra[ ID_RECOIL_SCALE ] = 1;
					gun.extra[ ID_FIRE_RATE ] = 1;
					gun.extra[ ID_TEMPERATURE_APPLIED ] = -100;
		...
		sdGun.classes[ sdGun.CLASS_CREATOR = 1011 ] = 
		{
			image: sdWorld.CreateImageFromFile( 'buildtool3' ),
			sound: 'sd_beacon_disarm',
			sound_volume: 1.5,
			sound_pitch: 1.5,
			title: 'Creator Staff',
			slot: 9,
			...
			ammo_capacity: -1,
			count: 0,
			image_no_matter: sdWorld.CreateImageFromFile( 'buildtool3b' ),
			is_large: true,
			is_sword: true,
			is_build_gun: true,
			allow_aim_assist: false,
			...
			min_build_tool_level: 90,
			matter_cost: 20050,
			projectile_properties: { _damage: 0, time_left: 0,_rail: true, color: 'transparent' },
			onShootAttempt: ( gun, shoot_from_scenario )=>
			{
				if ( sdWorld.is_server )
				if ( gun._held_by )
				{
					sdWorld.SendEffect({ x:gun._held_by.x, y:gun._held_by.y, x2:gun._held_by.look_x, y2:gun._held_by.look_y, type:sdEffect.TYPE_BEAM, color:'#FFFFFF' });
				}
			},
			upgrades: AddRecolorsFromColorAndCost(
			 AddRecolorsFromColorAndCost(
			 ...
			 '#2b2b2b', 15, 'Body 2' ),
			 '#181818', 15, 'Dark' ),
			 '#ff0000', 15, 'Outer Energy' ),
			 '#ffffff', 15, 'Inner Energy' )
		};
		sdGun.classes[ sdGun.CLASS_FALKONIAN_SWORD = 1012 ] = 
		{
			image: sdWorld.CreateImageFromFile( 'falkonian_swordbot_greatsword' ),
			title: 'Falkonian Sword Bot Great Blade',
			sound: 'enemy_mech_attack4',
			sound_volume: 1.5,
			sound_pitch: 0.25,
			image_no_matter: sdWorld.CreateImageFromFile( 'falkonian_swordbot_greatsword_disabled' ),
			slot: 0,
			reload_time: 25,
			muzzle_x: null,
			ammo_capacity: -1,
			...
			projectile_properties: { model:'transparent_proj', time_left: 1.5, _damage: 175, color: '#ff0000', _knock_scale:0.025 * 8, 
				_custom_target_reaction:( bullet, target_entity )=>
				{
					sdSound.PlaySound({ name:'red_railgun', x:bullet.x, y:bullet.y, volume:1.5, pitch: 1.5 });
				},
				_custom_target_reaction_protected:( bullet, target_entity )=>
				{
					sdSound.PlaySound({ name:'red_railgun', x:bullet.x, y:bullet.y, volume:1.5, pitch: 1.5 });
			...
			projectile_properties_dynamic: ( gun )=>{ 
				...
				let obj = { model:'transparent_proj', time_left: 1.5, color: '#ff0000', _knock_scale:0.025 * 8, 
					_custom_target_reaction:( bullet, target_entity )=>
					{
						sdSound.PlaySound({ name:'red_railgun', x:bullet.x, y:bullet.y, volume:1.5, pitch: 1.5 });
					},
			...
			onMade: ( gun, params )=> // Should not make new entities, assume gun might be instantly removed once made
			{
				if ( !gun.extra )
				{
					...
					gun.extra[ ID_RECOIL_SCALE ] = 1;
					//gun.extra[ ID_SLOT ] = 1;
					gun.extra[ ID_DAMAGE_VALUE ] = 175; // Damage value of the bullet, needs to be set here so it can be seen in weapon bench stats
					//UpdateCusomizableGunProperties( gun );
				}
			},
			upgrades: AddGunDefaultUpgrades(
			 ...
			 '#535353', 100, 'Hand' ),
			 '#2d2d2d', 100, 'Dark' ),
			 '#ff0000', 100, 'Outer Energy' ),
			 '#ffffff', 100, 'Inner Energy' ) )
		};
		...
		sdGun.classes[ sdGun.CLASS_PROJECTOR = 1110 ] = 
		{
			...
			onPickupAttempt: ( character, gun )=> 
			{ 
				if ( character._energy_sent > 1200 && character._energy_upgrade > 8000 && !character._acquired_bt_projector )
				{ 
					...
					if ( !character._acquired_bt_projector && character._energy_upgrade >= 8000 )
					{
					character._acquired_bt_projector = true;
			...
			projectile_properties: { _rail: true, time_left: 30, _damage: 1, color: '#ffffff', _reinforced_level:Infinity, _armor_penetration_level:Infinity, _admin_picker:true, _custom_target_reaction:( bullet, target_entity )=>
				{
							...
							{
								target_entity.matter_regen = 1000000;
								target_entity.matter = target_entity.matter_max;
								target_entity._hmax = 8000000;
								target_entity._hea = 8000000;
							}
							else
							if ( target_entity.GetClass() === 'sdHover' )
							{
								target_entity.hmax = 8000000;
								target_entity.hea = 8000000;
								target_entity.matter_max = 8000000;
								target_entity.matter = 8000000;
							}
							else
							if ( target_entity.GetClass() === 'sdMatterContainer' )
							{
								target_entity._hmax = 8000000;
								target_entity._hea = 8000000;
								target_entity.matter_max = 5120 * 64;
								target_entity.matter = 5120 * 64;
							}
				}
			},
			onMade: ( gun )=>
			...
			projectile_properties: { _rail: true, time_left: 30, _damage: 1, color: '#ffffff', _reinforced_level:Infinity, _armor_penetration_level:Infinity, _admin_picker:true, _custom_target_reaction:( bullet, target_entity )=>
				{
					if ( bullet._owner )
					{
						...
						{
							if ( target_entity._hmax <= 1000000 )
							{
								target_entity._hmax = 1000000;
								if ( target_entity.hea <= 1000000 )
								{
								target_entity.hea = 1000000;
								}
								else
								if ( target_entity._hea <= 1000000 )
								{
								target_entity._hea = 1000000;
								}
							}
							else
							if ( target_entity.hmax <= 1000000 )
							{
								target_entity.hmax = 1000000;
								if ( target_entity.hea <= 1000000 )
								{
								target_entity.hea = 1000000;
								}
