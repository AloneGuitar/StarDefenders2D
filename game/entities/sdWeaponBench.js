					...
					if ( command_name === 'UPGRADE' )
					{
						...
						if ( i >= 0 & i < upgrades.length )
						{
							if ( upgrades[ i ].title = "Increase damage" && upgrades[ i ].cost === 2 ) // Normal gun damage upgrade price
							{
								...
								slot_mult = 1.5;
								let matter_cost_durability = sdGun.classes[ this.item0.class ].matter_cost <= 150 ? ( sdGun.classes[ this.item0.class ].matter_cost * 2 || 60 ) : 300; // Matter cost for durability is either equal to cost to build or 300 for non-buildable items
